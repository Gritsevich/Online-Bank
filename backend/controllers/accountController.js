...
class AccountController
{
  async getList(req, res, next)
  {
    const accounts = await Accounts.findAll({
        ...
        attributes: ['id', 'name',[Sequelize.literal('CAST(amount AS DECIMAL(10, 2))'),'amount'], [Sequelize.col('currency.type'), 'currency']],
        include: [
            {
                model: Currency,
                attributes: []
  ...
  async create(req, res, next)
  {
    ...
    if(typeAccountId == TypeAccountEnum.CREDIT){
        await createCredit(creditAmount, term, accountId, next)
    }
